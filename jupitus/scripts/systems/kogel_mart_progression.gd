...
@export var intro_trigger_path: NodePath = (
	^"RobberyCutscene/Area3D"
)

@export var post_battle_cutscene_path: NodePath = (
	^"PostBattleCutscene"
)

@export var cultist_path: NodePath = (
	^"MachineCultist"
)

@export var clerk_path: NodePath = (
	^"KogelBotClerk"
)
...
@export var cultist_defeated_flag: StringName = (
	&"machine_cultist_defeated"
)
...
@export var battle_started_flag: StringName = (
	&"machine_cultist_battle_started"
)
...
@onready var _cultist := get_node_or_null(
	cultist_path
) as NPC

@onready var _clerk := get_node_or_null(
	clerk_path
) as NPC

@onready var _milk := get_node_or_null(
	milk_path
) as MilkInteractable
...
func _ready() -> void:
	...
	if not PlayerState.has_flag(
		cultist_defeated_flag
	):
		return
...
func _validate_scene_nodes() -> bool:
	...
	if _intro_trigger == null:
		push_error(
			"KogelMartProgression: intro trigger not found"
		)
		valid = false

	if _post_battle_cutscene == null:
		push_error(
			"KogelMartProgression: post-battle cutscene not found"
		)
		valid = false

	if _cultist == null:
		push_error(
			"KogelMartProgression: MachineCultist not found"
		)
		valid = false

	if _clerk == null:
		push_error(
			"KogelMartProgression: KogelBotClerk not found"
		)
		valid = false

	if _milk == null:
		push_error(
			"KogelMartProgression: MilkInteractable not found"
		)
		valid = false
