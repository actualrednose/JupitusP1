...
func setup(session: BattleSession) -> void:
	...
	if (
		_session != null
		and _session.action_resolved.is_connected(_on_action_resolved)
	):
		_session.action_resolved.disconnect(_on_action_resolved)

	_session = session

	if (
		_session != null
		and not _session.action_resolved.is_connected(_on_action_resolved)
	):
...
func unregister_view(combatant: CombatantState) -> void:
...
func present_power_attack_charge(
	...
	_play_audio(power_attack_charge_sound, _charge_audio)
...
func _on_action_resolved(action: BattleAction) -> void:
	...
	_present_action.call_deferred(action, _presentation_id)
...
func _present_action(
	...
	if action.ability != null and action.ability.is_power_attack:
	...
	if not await _wait(anticipation, presentation_id):
		return
	...
	_call_view(
		...
		[
			target_position,
			_scaled_tween_duration(travel_duration),
			reduced_motion
		]
	)

	if not await _wait(travel_duration, presentation_id):
		return
	...
	_call_view(
		...
		"play_recovery",
		[
			_scaled_tween_duration(recovery_duration),
			reduced_motion
		]
	)

	if not await _wait(recovery_duration, presentation_id):
		return

	_finish_presentation(action, presentation_id)
...
func _present_effect(
	...
	var target_view: Object = _views.get(result.target)

	if result.skipped or result.missed:
		var result_text := "MISS" if result.missed else "NO EFFECT"
		_call_view(
			target_view,
			"show_result_text",
			[result_text, Color("#E5E5E5"), reduced_motion]
		)
		return await _wait(result_duration, presentation_id)

	if not result.applied:
		_call_view(
			target_view,
			"show_result_text",
			["NO EFFECT", Color("#E5E5E5"), reduced_motion]
		)
		return await _wait(result_duration, presentation_id)

	match result.effect.effect_type:
		AbilityEffectDefinition.EffectType.DAMAGE:
			if not await _wait(hit_stop_duration, presentation_id):
				return false
			...
			_call_view(
				...
				[
					direction,
					_scaled_tween_duration(result_duration),
					reduced_motion,
					reduced_flashing
				]
			)
			...
			_play_audio(hit_sound, _effect_audio)
			_play_audio(hit_accent_sound, _accent_audio)
			_play_shake(direction, result.amount)
		...
		AbilityEffectDefinition.EffectType.HEAL, AbilityEffectDefinition.EffectType.REVIVE:
			...
			_play_audio(heal_sound, _effect_audio)
		...
		AbilityEffectDefinition.EffectType.GUARD:
			...
			_play_audio(guard_sound, _effect_audio)
		...
		AbilityEffectDefinition.EffectType.CLEANSE:
			_call_view(
				target_view,
				"show_result_text",
				["CLEANSED", Color("#A8F0FF"), reduced_motion]
			)
	...
	if result.tempo_after != result.tempo_before:
		_call_view(
			...
			"pulse_tempo",
			[
				result.tempo_after - result.tempo_before,
				_scaled_tween_duration(result_duration),
				reduced_motion,
				reduced_flashing
			]
	...
	if not await _wait(result_duration, presentation_id):
		return false
...
func _wait(
	...
	while remaining > 0.0:
		...
		remaining -= get_process_delta_time() * multiplier

	return _is_current(presentation_id)


func _is_current(presentation_id: int) -> bool:
	return _presenting and presentation_id == _presentation_id


func _find_target_view(action: BattleAction) -> Object:
	if action.target != null and _views.has(action.target):
		return _views[action.target]

	for result in action.effect_results:
		if result != null and _views.has(result.target):
			return _views[result.target]

	return null


func _get_view_position(view: Object) -> Vector2:
	if view != null and view.has_method("get_presentation_position"):
		return view.call("get_presentation_position") as Vector2
...
func _sync_all_views() -> void:
	for view: Object in _views.values():
		_call_view(view, &"sync_from_state", [])


func _stop_all_view_animation() -> void:
	for view: Object in _views.values():
		_call_view(view, &"skip_presentation", [])


func _play_shake(direction: Vector2, amount: int) -> void:
	if (
		shake_target == null
		or reduced_motion
		or camera_shake_strength <= 0.0
	...
	_stop_shake()
	var strength := minf(
		camera_shake_strength + float(amount) * 0.08,
	...
	for multiplier in [1.0, -0.65, 0.35, 0.0]:
		_shake_tween.tween_property(
			shake_target,
			"position",
			_shake_base_position
				+ shake_direction * strength * multiplier,
...
func _stop_shake() -> void:
	...
	if shake_target != null:
		shake_target.position = _shake_base_position
...
func _scaled_tween_duration(duration: float) -> float:
	var multiplier := animation_speed
