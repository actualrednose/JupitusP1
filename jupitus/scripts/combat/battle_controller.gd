...
func _ready() -> void:
	...
	battle_session.phase_changed.connect(
		_on_phase_changed
	)

	battle_session.action_queued.connect(
		_on_action_queued
	)

	battle_session.timing_requested.connect(
		_on_timing_requested
	)

	battle_session.action_started.connect(
		_on_action_started
	)

	battle_session.action_resolved.connect(
		_on_action_resolved
	)

	battle_session.action_cancelled.connect(
		_on_action_cancelled
	)

	battle_session.damage_applied.connect(
		_on_damage_applied
	)

	battle_session.power_attack_started.connect(
		_on_power_attack_started
	)

	battle_session.power_attack_disrupted.connect(
		_on_power_attack_disrupted
	)

	battle_session.battle_won.connect(
		_on_battle_won
	)

	battle_session.battle_lost.connect(
		_on_battle_lost
	)

	battle_session.setup(
		player_definitions,
		enemy_definitions
	)

	presentation_director.setup(
		battle_session
	)
...
func _on_power_attack_started(
	...
	presentation_director.present_power_attack_charge(
		enemy
	)
...
func start_battle() -> void:
	if battle_session == null:
		push_warning(
			"BattleController: battle session has not been created"
		)
		return
...
func set_presentation_fast_forwarding(
	...
	if presentation_director != null:
		presentation_director.set_fast_forwarding(
			value
		)
